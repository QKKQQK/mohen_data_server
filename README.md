...
tree | 是否以树形结构文件返回(只能返回文件) | 否 | Boolean | 树形结构不适用于聚合查询 |  | true 
...
tree_group_type | 树形结构聚合类型("max", "min", "sum", "avg") | 否(tree参数存在时为必需) | String[] | 无 |  | ["max", "avg", "sum"]
path | 树形结构路径字段名称("rlist", "klist", "extlist.attr") | 否(tree参数存在时为必需) | String | 无 |  | "rlist"
show_raw_data | 树形结构是否返回合并数据 | 否(tree参数存在时为必需) | Boolean | 无 |  | true 
data. | 查询数据条件参数 | 是 | 
